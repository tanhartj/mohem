...
export async function processLongVideo(job) {
  const { channel_id, niche } = JSON.parse(job.data);
  
  logger.info('Processing long video job', { jobId: job.id, channelId: channel_id, niche });
  ...
  try {
    ...
    logger.info('Step 1/5: Generating long-form content...');
    const content = await generateContent(niche, 'long');
    ...
    logger.info('Step 5/5: Long video completed!', { youtubeId: uploadResult.videoId });
    
    queries.updateJobCompleted.run('completed', Math.floor(Date.now() / 1000), job.id);
    
    sendMessage(`✅ *Long Video Published!*\n\n${content.titles[0]}\n\n${uploadResult.url}`);
    ...
    logger.error('Long video job failed', { jobId: job.id, error: error.message, stack: error.stack });
    queries.updateJobCompleted.run('failed', Math.floor(Date.now() / 1000), job.id);
    
    sendMessage(`❌ *Long Video Failed*\n\nError: ${error.message}`);
    
    throw error;
  }
}

export default { processLongVideo };
